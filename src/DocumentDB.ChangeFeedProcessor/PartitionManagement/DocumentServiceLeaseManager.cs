...
namespace Microsoft.Azure.Documents.ChangeFeedProcessor.PartitionManagement
{
    ...
    internal class DocumentServiceLeaseManager : ILeaseManager
    {
        ...
        public async Task<ILease> CreateLeaseIfNotExistAsync(string partitionId, string continuationToken)
        {
            ...
            if (created)
            {
                Logger.InfoFormat("[{1}] Created lease for partition {0}.", partitionId, this.leaseStoreCollectionInfo.CollectionName);
                return documentServiceLease;
            }

            Logger.InfoFormat("[{1}] Some other host created lease for {0}.", partitionId, this.leaseStoreCollectionInfo.CollectionName);
            return null;
        }
        ...
        public async Task<ILease> CheckpointAsync(ILease lease, string continuationToken)
        {
            ...
            return await this.leaseUpdater.UpdateLeaseAsync(
                ...
                serverLease =>
                {
                    ...
                    {
                        Logger.InfoFormat("[{2}] Partition {0} lease was taken over by owner '{1}'", lease.PartitionId, serverLease.Owner, this.leaseStoreCollectionInfo.CollectionName);
        ...
        public async Task<ILease> RenewAsync(ILease lease)
        {
            ...
            {
                Logger.InfoFormat("[{1}] Partition {0} failed to renew lease. The lease is gone already.", lease.PartitionId, this.leaseStoreCollectionInfo.CollectionName);
                throw new LeaseLostException(lease);
            }
            ...
            return await this.leaseUpdater.UpdateLeaseAsync(
                ...
                serverLease =>
                {
                    if (serverLease.Owner != lease.Owner)
                    {
                        Logger.InfoFormat("[{1}] Partition {0} lease was taken over by owner '{1}'", lease.PartitionId, serverLease.Owner, this.leaseStoreCollectionInfo.ConnectionPolicy);
                        throw new LeaseLostException(lease);
                    }
                    return serverLease;
                }).ConfigureAwait(false);
        ...
        public async Task ReleaseAsync(ILease lease)
        {
            ...
            if (refreshedLease == null)
            {
                Logger.InfoFormat("[{1}] Partition {0} failed to release lease. The lease is gone already.", lease.PartitionId, this.leaseStoreCollectionInfo.ConnectionPolicy);
                throw new LeaseLostException(lease);
            }
            ...
            await this.leaseUpdater.UpdateLeaseAsync(
                ...
                serverLease =>
                {
                    if (serverLease.Owner != lease.Owner)
                    {
                        Logger.InfoFormat("[{2}] Partition {0} no need to release lease. The lease was already taken by another host '{1}.", lease.PartitionId, serverLease.Owner, this.leaseStoreCollectionInfo.ConnectionPolicy);
                        throw new LeaseLostException(lease);
                    }
        ...
        public async Task<ILease> UpdatePropertiesAsync(ILease lease)
        {
            ...
            {
                Logger.InfoFormat("[{2}] Partition '{0}' lease was taken over by owner '{1}' before lease properties update", lease.PartitionId, lease.Owner, this.leaseStoreCollectionInfo.CollectionName);
                throw new LeaseLostException(lease);
            }
